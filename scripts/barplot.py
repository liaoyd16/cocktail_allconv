...
bounds = [[1e-2,4,-1,0],[1,np.inf,1,1]]

def do_many_bars(bar_vals, f1, f2, p01, p02): #size 2x5
    ...
    print(r_1x, r_2x)
    popt1, pcov1 = curve_fit(f1, np.arange(5), r_1x, p0=p01, bounds=bounds, maxfev=10000)
    popt2, pcov2 = curve_fit(f2, np.arange(5), r_2x, p0=p02, bounds=bounds, maxfev=10000)
...
def many_bars(cumu_single, redump):
    modelname = f"../results/dumped/n={n}_en={k}_resblock_3x3conv_5shortcut/de=en"
    which_figure = {'single': 5, 'cumu': 4}[cumu_single]

    if redump:
        bar_vals_1x = [[],[]]
        bar_vals_2x = [[],[]]
        for ijson in range(1,6):
            print(ijson)
    ...
    p0_sigmoid1 = cheat[cumu_single]['1x'][0]
    p0_sigmoid2 = cheat[cumu_single]['1x'][1]

    i = 0
    while i < MAXITER:
        try:
            do_many_bars(bar_vals_1x, sigmoid, sigmoid, p01=p0_sigmoid1, p02=p0_sigmoid2)
            plt.savefig(f"../results/figures/figure{which_figure}/barplot_1x_{cumu_single}.png", dpi = 300, bbox_inches='tight')
            break
        except:
            print(i)
            i += 1
            plt.close()
    if i == MAXITER: print("bar_vals_1x failed!")

    p0_sigmoid1 = cheat[cumu_single]['2x'][0]
    p0_sigmoid2 = cheat[cumu_single]['2x'][1]
    i = 0
    while i < MAXITER:
        try:
            do_many_bars(bar_vals_2x, sigmoid, sigmoid, p01=p0_sigmoid1, p02=p0_sigmoid2)
            plt.savefig(f"../results/figures/figure{which_figure}/barplot_2x_{cumu_single}.png", dpi = 300, bbox_inches='tight')
            break
        except:
            print(i)
            i += 1
            plt.close()
    if i == MAXITER: print("bar_vals_2x failed!")


if __name__ == '__main__':
    #"""
    # figure 3
    bar(f"../results/dumped/n={n}_en={k}_resblock_3x3conv_5shortcut/de=en/", "condition_cumu-tpdwn_5_spec.json")
    plt.savefig("../results/figures/figure3/bar.png", dpi=300, bbox_inches='tight')
    """
    # figure 4/5
    cumu_single = 'cumu'
    redump = True
    many_bars(cumu_single, redump)
    #"""
