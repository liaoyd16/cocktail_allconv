...
class ResAE(nn.Module):
    def __init__(self, keepsize_num_en, keepsize_num_de, shortcut):
        ...
        self.upward_net2 = nn.Sequential(
            ...
            nn.Conv2d(in_channels=8, out_channels=8, padding=(0,0), stride=(3,3), kernel_size=(8,5), dilation=(2,2)),
            nn.BatchNorm2d(8),
            nn.ReLU(),
            ResBlock(8, 12),
        ...
        self.upward_net3 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear'),
            nn.Conv2d(in_channels=12, out_channels=12, padding=(0,0), stride=(3,3), kernel_size=(5,3), dilation=(2,2)),
            nn.BatchNorm2d(12),
            nn.ReLU(),
            ResBlock(12, 16),
        ...
        self.upward_net4 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear'),
            nn.Conv2d(in_channels=16, out_channels=16, padding=(0,0), stride=(3,3), kernel_size=(4,3), dilation=(2,2)),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            ResBlock(16, 24),
        ...
        self.upward_net5 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear'),
            nn.Conv2d(in_channels=24, out_channels=24, padding=(0,1), stride=(3,3), kernel_size=(2,3), dilation=(2,2)),
            nn.BatchNorm2d(24),
            nn.ReLU(),
            ResBlock(24, 32),
        ...
        self.upward_net6 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear'),
            nn.Conv2d(in_channels=32, out_channels=32, padding=(0,1), stride=(3,3), kernel_size=(2,3), dilation=(2,2)),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            ResBlock(32, 48),
        ...
        self.upward_net7 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear'),
            nn.Conv2d(in_channels=48, out_channels=48, padding=(0,1), stride=(3,3), kernel_size=(2,3), dilation=(2,2)),
            nn.BatchNorm2d(48),
            nn.ReLU(),
            ResBlock(48, 64),
        ...
        self.upward_net8 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear'),
            nn.Conv2d(in_channels=64, out_channels=64, padding=(0,1), stride=(3,3), kernel_size=(2,3), dilation=(2,2)),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            ResBlock(64, 128),
        ...
        self.upward_net9 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear'),
            nn.Conv2d(in_channels=128, out_channels=128, padding=(0,1), stride=(3,3), kernel_size=(2,3), dilation=(2,2)),
            nn.BatchNorm2d(128),
            nn.ReLU(),
            ResBlock(128, 128),
            *[ResBlock(128, 128) for _ in range(keepsize_num_en)]
        ...
        self.downward_net9 = nn.Sequential(
            *[ResBlock(128, 128) for _ in range(keepsize_num_de)],
            ResBlock(128, 128),
            nn.ConvTranspose2d(in_channels=128, out_channels=128, padding=(0,1), stride=(3,3), kernel_size=(2,3), dilation=(2,2)),
            nn.BatchNorm2d(128),
            nn.ReLU(),
        ...
        self.downward_net8 = nn.Sequential(
            *[ResBlock(128, 128) for _ in range(keepsize_num_de)],
            ResBlock(128, 64),
            nn.ConvTranspose2d(in_channels=64, out_channels=64, padding=(0,1), stride=(3,3), kernel_size=(2,3), dilation=(2,2)),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        ...
        self.downward_net7 = nn.Sequential(
            *[ResBlock(64, 64) for _ in range(keepsize_num_de)],
            ResBlock(64, 48),
            nn.ConvTranspose2d(in_channels=48, out_channels=48, padding=(0,1), stride=(3,3), kernel_size=(2,3), dilation=(2,2)),
            nn.BatchNorm2d(48),
            nn.ReLU(),
        ...
        self.downward_net6 = nn.Sequential(
            *[ResBlock(48, 48) for _ in range(keepsize_num_de)],
            ResBlock(48, 32),
            nn.ConvTranspose2d(in_channels=32, out_channels=32, padding=(0,1), stride=(3,3), kernel_size=(2,3), dilation=(2,2)),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=(2,2), stride=(2,2), padding=(1,1))
        )
        ...
        self.downward_net5 = nn.Sequential(
            *[ResBlock(32, 32) for _ in range(keepsize_num_de)],
            ResBlock(32, 24),
            nn.ConvTranspose2d(in_channels=24, out_channels=24, padding=(0,1), stride=(3,3), kernel_size=(2,3), dilation=(2,2)),
            nn.BatchNorm2d(24),
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=(2,2), stride=(2,2), padding=(1,1)),
        )
        ...
        self.downward_net4 = nn.Sequential(
            *[ResBlock(24, 24) for _ in range(keepsize_num_de)],
            ResBlock(24, 16),
            nn.ConvTranspose2d(in_channels=16, out_channels=16, padding=(0,0), stride=(3,3), kernel_size=(4,3), dilation=(2,2)),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=(2,2), stride=(2,2), padding=(1,1))
        )
        ...
        self.downward_net3 = nn.Sequential(
            *[ResBlock(16, 16) for _ in range(keepsize_num_de)],
            ResBlock(16, 12),
            nn.ConvTranspose2d(in_channels=12, out_channels=12, padding=(0,0), stride=(3,3), kernel_size=(5,3), dilation=(2,2)),
            nn.BatchNorm2d(12),
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=(2,2), stride=(2,2), padding=(1,1))
        )
        ...
        self.downward_net2 = nn.Sequential(
            *[ResBlock(12, 12) for _ in range(keepsize_num_de)],
            ResBlock(12, 8),
            nn.ConvTranspose2d(in_channels=8, out_channels=8, padding=(0,0), stride=(3,3), kernel_size=(8,5), dilation=(2,2)),
            nn.BatchNorm2d(8),
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=(2,2), stride=(2,2), padding=(1,1)),
        )
    ...
    def upward(self, specgrams, attentions):                # 1, 256, 128  # F.relu() bracket movement 19.12.5
        specgrams = specgrams.view(-1, 1, *Meta.data_meta['specgram_size'])
